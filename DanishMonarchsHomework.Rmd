...
# YOUR CODE
kings %>%
  mutate(duration=`End of reign`-`Start of reign`,na.rm=TRUE)
```
...
```{r}
# YOUR CODE
mean(kings$`Years of reign`, na.rm = TRUE)
...
```{r}
# YOUR CODE
filtered_kings <- kings%>%
  filter(`Years of reign`>20.21154)
print(filtered_kings)
count(filtered_kings)

# 24 kings with a reign over the average 20.2 years.
```
